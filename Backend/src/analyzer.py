...
{reviews_block}"""
    ...
    try:
        ...
        return {
            "praised_features": [],
            "missing_features": [],
            "sentiment_summary": "Could not parse analysis.",
            "competitive_notes": "",
        }


async def generate_competitive_report(keyword: str, analyses: list) -> dict:
    """
    Synthesize all individual app analyses into a final competitive report.

    Returns a dict with:
      - competitive_report: full Markdown string
      - top_valued_features: list of strings
      - common_pain_points: list of strings
      - differentiation_opportunities: list of strings
      - quick_wins: list of strings
    """
    if not analyses:
        empty = "No apps were analyzed."
        return {
            "competitive_report": empty,
            "top_valued_features": [],
            "common_pain_points": [],
...
{{
  "top_valued_features": ["concise string per bullet, plain text, no markdown"],
  "common_pain_points": ["concise string per bullet, plain text, no markdown"],
  "differentiation_opportunities": ["concise string per bullet, plain text, no markdown"],
  "quick_wins": ["concise string per bullet, plain text, no markdown"],
  "competitive_report": "the full Competitive Intelligence Report in Markdown format with all four sections as ## headings and bullet points"
}}

Rules:
- Each array item is a single self-contained insight (1-2 sentences max).
